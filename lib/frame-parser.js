...
frame_parser.parseNodeIdentificationPayload = function(frame, buffer) {
  ...
  // Calculate the length of the NI string
  var offset = 10;
  var niLen = 0;
  while(buffer[offset + niLen++] !== 0x00);

  // Extract the NI string from the buffer
  frame.nodeIdentifier = buffer.toString('ascii', offset, offset + niLen - 1).trim();
  offset += niLen;

  frame.remoteParent16 = frame_parser.parseAddress(buffer, offset, 2);
  frame.deviceType = buffer.readUInt8(offset+2);
  frame.sourceEvent = buffer.readUInt8(offset+3);
  frame.digiProfileID = frame_parser.parseAddress(buffer, offset+4, 2);
  frame.digiManufacturerID = frame_parser.parseAddress(buffer, offset+6, 2);
};
