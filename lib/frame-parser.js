...
frame_parser[C.FRAME_TYPE.XBEE_SENSOR_READ] = function(frame, buffer) {
  ...
  var data = buffer.slice(12).toJSON();
  frame.sensorValues = {
      AD0: Math.round(100 * (((data[0] << 8) | data[1]) * 5.1) / 255.0) / 100,
      AD1: Math.round(100 * (((data[2] << 8) | data[3]) * 5.1) / 255.0) / 100,
      AD2: Math.round(100 * (((data[4] << 8) | data[5]) * 5.1) / 255.0) / 100,
      AD3: Math.round(100 * (((data[6] << 8) | data[7]) * 5.1) / 255.0) / 100,
      T:   (data[8] << 8) | data[9],
...
frame_parser[C.FRAME_TYPE.RX_PACKET_64] = function(frame, buffer) {
  ...
  frame.data = buffer.slice(10).toJSON();
};
...
frame_parser[C.FRAME_TYPE.RX_PACKET_16] = function(frame, buffer) {
  ...
  frame.data = buffer.slice(4).toJSON();
};
