...
function XBeeAPI(options) {
  ...
  var self = this;
  this.encoder = new stream.Transform( { objectMode: true } );
  this.encoder._transform = function (frame, enc, cb) {
    self.encoder.push(self.buildFrame(frame));
    cb();
  }

  this.decoder = new stream.Transform( { objectMode: true } );
  this.decoder._transform = function (chunk, enc, cb) {
  ...
  for(var i = 0; i < buffer.length; i++) {
    ...
    if (S.length > 0 && S.offset === S.length + 4) {
      ...
      if (this.options.raw_frames || !this.canParse(rawFrame)) {
        if (cb !== undefined && typeof(cb) == 'function') this.decoder.push(rawFrame);
        else this.emit("frame_raw", rawFrame);
      } else {
        var frame = this.parseFrame(rawFrame);
        if (cb !== undefined && typeof(cb) == 'function') this.decoder.push(frame);
